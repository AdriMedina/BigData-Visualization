                ...
                function isLoadedData(fin) {
                    if (!fin) {
                        ...
                        Result.find({
                            ...
                            if (err) return console.error(err);
                            res.send(get_bubblechart(JSON.stringify(dataMongo[0])).node().outerHTML);
                            db.close(function() {
            ...
            if (config.api.exec_mode == "systemcall") {
                ...
                function getResultAndSend(fin) {
                    if (!fin) {
                         ...
                         *
                         */
                        var access = fs.createWriteStream('./graphicRegister/bubblechart.time.log', {
                            flags: 'a',
                            autoClose: true
                        });
                        process.stdout.write = process.stderr.write = access.write.bind(access);
                        console.timeEnd('bubblechart');
                ...
                //          *
                //          */
                //          var access = fs.createWriteStream('./graphicRegister/bubblechart.time.log', { flags: 'a', autoClose: true });
                //          process.stdout.write = process.stderr.write = access.write.bind(access);
                //          console.timeEnd('bubblechart');
                //          console.log('File: ' + file);
                //          console.log('ColX: ' + colx);
...
function get_bubblechart(dataJSON) {
    ...
    /*var maxX = d3.max(json.values, function(d) {return d.pointX});
    var minX = d3.min(json.values, function(d) {return d.pointX});
    var maxY = d3.max(json.values, function(d) {return d.pointY});
    var minY = d3.min(json.values, function(d) {return d.pointY});
    x.domain([minX - (minX*0.5), maxX + (maxX*0.5)]);
    y.domain([minY - (minY*0.5), maxY + (maxY*0.5)]);*/
    ...
    svg.selectAll(".node")
        ...
        .attr("transform", function(d) {
            return "translate(" + x(d.pointX) + "," + y(d.pointY) + ")";
        })
        .append("circle")
        .attr("r", function(d) {
            return (d.valueColor) * 3;
        })
